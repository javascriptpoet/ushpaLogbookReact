...
by invoking it. <br>
At the beginning, there was just one particle, a supemassiv lepton of a function - the createApp. By itself typeless, no id
required, a lone wolf. in a timeless time it spawned others, they are all Apps - lighter but still heavy barrions, value elements.
They were born by invoking createApp. Well, i put the dirs with my apps in them and ran createApp in each index file.
Within each app folder is the universe of complexity but on that top scope level, there is just barrion apps and the typeless
createApp law giver. 
